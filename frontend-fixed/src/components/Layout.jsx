...
import { LogOut, Users2, ShieldCheck, ListChecks, ServerCog, Boxes } from 'lucide-react'
        ...
        <nav className="p-3 space-y-1">
          {isAdmin ? (
            <>
              ...
              <NavLink to="/admin/farmers" className={({isActive})=>`block px-3 py-2 rounded-lg ${isActive?'bg-indigo-50 text-indigo-700':'hover:bg-gray-50'}`}>
                <ListChecks className="inline mr-2 h-4 w-4"/> Farmers
              </NavLink>
              <NavLink to="/admin/verify-all" className={({isActive})=>`block px-3 py-2 rounded-lg ${isActive?'bg-indigo-50 text-indigo-700':'hover:bg-gray-50'}`}>
                <ShieldCheck className="inline mr-2 h-4 w-4"/> Verify All
              </NavLink>
            </>
          ) : (
            <>
